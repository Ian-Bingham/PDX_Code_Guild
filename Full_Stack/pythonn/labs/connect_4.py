...
def read_moves(filename):
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for move in f.readlines():
            moves_list.append(int(move.rstrip()) - 1)
...
def main():
    ...
    player_turn = 0
    while True:
        move = input("What column would '{}' like to place a piece?: ".format(player[player_turn]))
        if move.isdigit():
            if int(move) - 1 in range(7):
                board = make_move(board, int(move) - 1, player[player_turn])
                print_board(board)
                if check_win(board, player[player_turn]):
                    print("Congrats, {}. You won!!!".format(player[player_turn]))
                    exit(0)
