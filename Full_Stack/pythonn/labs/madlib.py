...
def madlib():
    # assumes user types correct input EVERY time
    ...
    # shuffle each list to randomize madlib story
    ...
    story_time = input("Would you like to hear the story I "
                       "created (again)?\n> ")

    while True:
        if story_time.lower() == "yes" or story_time.lower() == "y":
            print(
                    f"""One day my friend {names_list[0]} and I went to the
                    {places_list[0]}. He was wearing a {adjectives_list[0]}
                    coat and {adjectives_list[1]} shoes while I wore nothing
                    but {adjectives_list[2]} underwear. Once there we met up
                    with our other friend {names_list[1]} and ate some
                    {foods_list[0]}. It was delicious! To burn off the
                    calories we planned to go to {places_list[1]} and do some
                    {verbs_list[0]}, but instead we sat around and ate some
                    {foods_list[1]}... We'll make up for it by going
                    {verbs_list[1]} tomorrow."""
            )
